...
const UserTableItem = ({ userList }) => {
  ...
  const confirmUserDel = async() => {
        ...
        handleClose();
        const res = await axios.delete(`${SERVERHTTP}admin/del`, { data: { id: itemState.id } })
       console.log(res);
      ...
      <>
        {userList.map((item, key) => (
          <tr key={item.id}>
            ...
            <td style={{ alignItems: 'center' }}>
              ...
              <i
                className={styles.tableTrash}
                onClick={() => userDel(item)}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Footer>
            ...
            <Button variant="primary" onClick={confirmUserDel}>
              Confirm
            </Button>
          </Modal.Footer>
        </Modal>
